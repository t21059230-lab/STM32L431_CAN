...
class SensorCollector(private val context: Context) : SensorEventListener, LocationListener {
    ...
    // External GPS Service (set by MainActivity)
    var externalGpsService: SerialGpsService? = null
    ...
    fun updateBattery() {
        try {
            // Android 13+ (TIRAMISU) requires flags for registerReceiver
            // ACTION_BATTERY_CHANGED is a protected sticky broadcast, so we use RECEIVER_EXPORTED
    ...
    fun updateTelemetry() {
        ...
        // GPS: Use external GPS if connected, otherwise use phone GPS
        val extGps = externalGpsService?.lastGpsResult
        if (extGps != null && extGps.isValid && externalGpsService?.isConnected == true) {
            // Use External GPS (KCA Protocol)
            val nav = extGps.navData
            val speedHorizontal = sqrt(
                extGps.velocityNorth * extGps.velocityNorth + 
                extGps.velocityEast * extGps.velocityEast
            ).toFloat()
            val headingCalc = atan2(extGps.velocityEast, extGps.velocityNorth)
                .let { Math.toDegrees(it).toFloat() }
                .let { if (it < 0) it + 360f else it }
            
            TelemetryStreamer.updateGPS(
                nav.latitude.toDouble(),
                nav.longitude.toDouble(),
                nav.altitude,
                speedHorizontal,
                headingCalc,
                nav.totalUsedSatellites,
                if (nav.isValid) 3 else 0,  // 3D fix if valid
                (nav.hdop.toInt() and 0xFF) / 10f  // Decompress HDOP
            )
        } else {
            // Fallback: Use Phone GPS
            TelemetryStreamer.updateGPS(
