...
object TelemetryStreamer {
    ...
    var s1Fb: Float = -999.9f
    var s2Fb: Float = -999.9f
    var s3Fb: Float = -999.9f
    var s4Fb: Float = -999.9f
     ...
     */
    fun connect(usbManager: UsbManager, context: Context): Boolean {
        if (isConnected) {
        ...
        try {
            ...
            // Find FTDI device specifically (VID=0x0403)
            val ftdiDevice = deviceList.values.find { it.vendorId == 0x0403 }
            
            if (ftdiDevice == null) {
                Log.w(TAG, "No FTDI device found (VID=0x0403)")
                return false
            }
            
            val deviceName = ftdiDevice.deviceName
            val vid = ftdiDevice.vendorId
            val pid = ftdiDevice.productId
            
            Log.d(TAG, "Found FTDI device: $deviceName, VID=${String.format("0x%04X", vid)}, PID=${String.format("0x%04X", pid)}")
            
            // Check permission
            if (!usbManager.hasPermission(ftdiDevice)) {
                ...
                usbManager.requestPermission(ftdiDevice, intent)
                return false
            }
            
            // Find driver for FTDI
            val defaultProber = UsbSerialProber.getDefaultProber()
            val driver = defaultProber.probeDevice(ftdiDevice)
            
            if (driver == null) {
                Log.e(TAG, "No driver found for FTDI device")
                return false
            }
                
            // Try to open this device
            val connection = usbManager.openDevice(ftdiDevice)
            if (connection == null) {
    ...
    private fun reconnect() {
        ...
        disconnect()
        Thread.sleep(500)  // Wait before reconnecting
